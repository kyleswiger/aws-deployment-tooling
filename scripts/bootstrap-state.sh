#!/usr/bin/env bash
# Create the S3 bucket and DynamoDB lock table that hold Terraform remote state.
#
...
Done. Reference this backend from your Terraform (main.tf):
...
Or, for the profile-driven deploy.sh pattern, put the same values in the
profile's backend.hcl and pass -backend-config=backend.hcl at init.
EOF
